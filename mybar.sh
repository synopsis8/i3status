...
#  separator $bg "#3949AB"
  ...
  echo -n ",{"
#  echo -n "\"full_text\":\" $name  $line \","
  echo -n "\"full_text\":\" $(${HOME}/.config/i3status/refreshActiveWindow.sh)\","
...
mycurrency() {
  ...
  separator $bg "#546E7A"
  echo -n ",{"
  echo -n "\"name\":\"id_currency\","
  echo -n "\"full_text\":\" $(${HOME}/.config/i3status/currency.py) \","
  echo -n "\"color\":\"#000000\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
...
myip_public() {
  ...
  echo -n "\"full_text\":\" $(${HOME}/.config/i3status/ip.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
meteo() {
  ...
  echo -n "\"full_text\":\" $(${HOME}/.config/i3status/meteo.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
while read line;
do
  # echo $line > ${HOME}/gitclones/github/i3/tmp.txt
  # {"name":"id_vpn","button":1,"modifiers":["Mod2"],"x":2982,"y":9,"relative_x":67,"relative_y":9,"width":95,"height":22}


  # VPN click
  if [[ $line == *"name"*"id_vpn"* ]]; then
    lxterminal -e ${HOME}/.config/i3status/click_vpn.sh &
#    alacritty -e ${HOME}/.config/i3status/click_vpn.sh &

  # CHECK UPDATES
  elif [[ $line == *"name"*"id_systemupdate"* ]]; then
    lxterminal -e ${HOME}/.config/i3status/click_checkupdates.sh &
  ...
  # TIME
    ...
    lxterminal -e ${HOME}/.config/i3status/click_time.sh &

  # METEO
  elif [[ $line == *"name"*"id_meteo"* ]]; then
    xdg-open https://openweathermap.org/city/2925533 > /dev/null &
